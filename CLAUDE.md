...
Phase 4 complete — version back-test gate PASSED. Run `24766226453` (3-cell matrix `.github/workflows/phase4-probe.yml`, frozen behind `.phase4-probe-trigger`) built SBFspot V3.9.10, V3.9.11, V3.9.12 against the current Phase 3.3 + 3.4 pipeline on `arm × bookworm × sqlite`. **Residual vs upstream is identical across all three tags**: SBFspot `−16,384 B`, SBFspotUploadDaemon `−4,096 B`. Confirms the arm-bookworm irreducible is a property of upstream's toolchain setup, not a V3.9.12-specific quirk. Bar 1a' (pipeline-internal reproducibility across source tags) met: our daemon byte-matches across V3.9.10↔V3.9.11 where the effective source didn't change; upstream's daemon byte-matches across V3.9.11↔V3.9.12 (their different change-point, but same determinism property). Full matrix + per-binary sha256 table in `docs/phase4-baseline.md`, including a post-Phase-4 addendum that pinpoints the arm-bookworm ~16 KB residual to a specific mechanism: same function set as upstream (2,278 vs 2,279 functions), same EHABI personality routines (PR0/PR1/PR2/generic counts within ±4), but upstream's per-function unwind tables carry ~11 bytes more each. Delta concentrates in addresses `0x10000–0x5ffff` (the libstdc++ region of `.text`) with the app-code region ~zero-delta. Root cause narrowed from "libstdc++.a bytes differ" to "upstream's libstdc++.a was compiled with a gcc configure that emits more elaborate per-function EH tables" — an artefact of upstream's Canadian-cross gcc configure we don't have access to. Phase 4's version-axis outcome is orthogonal to the DB/codename/arch axes from Phase 3.5 — Phase 5 matrix expansion can proceed keyed on V3.9.12 as the release target. **Next gate: Phase 5 — incremental matrix expansion of release.yml (5a bullseye, 5b buster, 5c arm64, 5d nosql, 5e mariadb), using the Phase 3.5 per-combo LDFLAGS / prefix / packages table.**
