...
Phase 3.4 complete — Bar 1a'' reached on 14 of 15 combos. Probe run `24763117532` (`arm × bullseye × sqlite`) produces **normalised-ELF byte-identical binaries with upstream V3.9.12**: `SBFspot.norm` sha `61c8e6467ddc...`, `SBFspotUploadDaemon.norm` sha `431b9be3869a...`, both matching upstream exactly. Top-level diffoscope is 24 lines, all accounted for by the three Decision 1a accepted irreducibles (gzip wrapper, binary mtimes, build-id hash). The technique: (i) a `/usr/local/bin/g++` wrapper in the chroot that appends `-fmacro-prefix-map=/usr/include=d:\rpi\cross\<codename>[64]\gcc<ver>\<triplet>\sysroot\usr\include` to rewrite boost's `__FILE__` expansions to match upstream's Windows-cross-compile paths; (ii) drop `-static-libstdc++` and `-Wl,--build-id=none` from LDFLAGS (bullseye dynamic-links libstdc++ and ships a build-id); (iii) keep the Phase 3.3 `-Wl,-Bstatic -lbluetooth -Wl,-Bdynamic` static-bluetooth trick. Audit of all 15 upstream tarballs confirmed upstream's prefix scheme: `d:\rpi\cross\<codename>\gcc<ver>\<triplet>\sysroot\usr\include`, with arm64 codenames carrying a `64` suffix; a Phase 5 matrix will key on this table. **Revised Bar 1a'': achievable on all combos EXCEPT `arm × bookworm`** (any DB), which retains a permanent ~16 KB irreducible from static-libstdc++ object-byte drift between upstream's Windows-cross sysroot and any public Raspbian armhf libstdc++.a — documented in `docs/phase3.3-baseline.md`. The Windows-CI-as-an-alternative path was investigated and documented out-of-scope: no pre-built Windows toolchain matches upstream's `.comment`, the maintainer's crosstool-ng config isn't published, and the hardest sub-problem (libstdc++.a bytes) is exactly what our prefix-map wrapper sidesteps for free. Full writeup in `docs/phase3.4-baseline.md`. Probe workflow `.github/workflows/bullseye-probe.yml` frozen behind `.bullseye-probe-trigger` for reference. release.yml still targets `arm × bookworm × sqlite` at the Phase 3.3 state; applying the wrapper there is cosmetic (saves 480 B, residual stays ~16 KB) and deferred to Phase 5 matrix parameterisation. **Next gate: Phase 4 — back-test V3.9.11 / V3.9.10 against Bar 1a' on the current pipeline.**
...
- `docs/phase3.4-baseline.md` — Phase 3.4 end state: `-fmacro-prefix-map` wrapper achieves full Bar 1a'' byte-match on `arm × bullseye × sqlite`. Identifies upstream's per-combo prefix scheme; scopes Windows-CI out-of-scope. 14 of 15 combos now byte-reproducible via the same technique.
