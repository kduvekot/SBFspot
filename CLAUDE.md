...
Phase 5e complete — modern hardened pipeline, 15 cells, reproducible tarballs + debug sidecars. Active workflow: `.github/workflows/phase5e-probe.yml`, full rationale in companion `.github/workflows/phase5e-probe.md`. The pivot off chasing upstream byte-match (documented through Phase 5d) opened up modernising the pipeline: PIE + full RELRO + BIND_NOW + `_FORTIFY_SOURCE=2` + `-fstack-protector-strong` + `-D_GLIBCXX_ASSERTIONS` + `--build-id=sha1`, with PIC libbluetooth.a rebuilt per cell from distro bluez source so static-linking into PIE emits no TEXTREL on armhf. Runtime-dep surface matches upstream exactly (libbluetooth not in NEEDED; libmariadb dynamic). All 15 cells produce two reproducible artefacts: `sbfspot-<db>-<arch>-linux-<codename>.tar.gz` (main, stripped binaries + data files) and `sbfspot-<db>-<arch>-linux-<codename>.debug.tar.gz` (DWARF sidecars for symbolication) — both with deterministic tar (ustar, sorted, uid/gid 0, `--mtime=@$SOURCE_DATE_EPOCH` derived from V3.9.12 tag commit) + deterministic gzip (`gzip -n`). Debug-split via objcopy `--only-keep-debug` / `--strip-unneeded` / `--add-gnu-debuglink`; stripped binary gains a `.gnu_debuglink` section pointing gdb/debuginfod at the sidecar. Run-to-run reproducibility proven on all 30 artefacts: runs `24842197467` and `24842590751` produced byte-identical SHA256s. Smoke-test step (binary exec + readelf hardening verification) passes on every cell. Known limitation: arm cells pull from live Raspbian archive (no working snapshot service per Phase 0), so long-term reproducibility holds modulo Raspbian security updates; arm64 cells fully snapshot-pinned. Phase 5 (byte-match matrix run `24789691360`) preserved as frozen probe `phase5-matrix.yml` for reference; its residual analysis lives in `docs/phase5-baseline.md`.
