...
Phase 5 complete — 15-combo matrix expansion run. Final run `24789691360` (`.github/workflows/phase5-matrix.yml`, frozen behind `.phase5-matrix-trigger`) produces all 15 upstream V3.9.12 variants in parallel, 5 min wall-clock. **7 of 25 binaries are byte-identical with upstream** (all four `*-bullseye × {sqlite,nosql}` cells clean, plus the `sqlite-arm64-bookworm` daemon). Remaining 18 binaries fall into documented categories: 6 "size-match, content differs" (buster sysroot drift + the arm64-bookworm dual-`.comment` cases); 3 small residuals < 100 B (NEEDED ordering); 5 medium 4–8 KB (mariadb `libmariadbclient.a` byte drift); and the 4 arm-bookworm bookworm-specific residuals (−12–16 KB static-libstdc++, already characterised in Phase 4). One anomaly: `nosql-arm64-bookworm` shows −65 KB but it's **file-layout padding** (LOAD-segment file-offset-vs-vaddr alignment) not content — actual section-level delta is 9,680 B (the known dual-`.comment`). Phase 5 surfaced 2 real pipeline bugs already fixed (`sqlite-arm-buster` missing `libgnutls28-dev`; `mariadb-arm-buster` needs `-lgnutls` not `-lssl -lcrypto` because buster libmariadbclient uses gnutls). All per-combo LDFLAGS/pkgs/prefix/mirror consolidated in `docs/phase5-baseline.md`. Phase 5 does **not** yet include per-variant tarball assembly (Phase 5b) or rootfs caching — matrix probe focused on binary residual measurement.
