...
Phase 3 closed. Phase 3.3 (run `24761451074`) added `-Wl,-Bstatic -lbluetooth -Wl,-Bdynamic` to SBFspot's LDFLAGS after a per-section diff localised the 110 KB upstream gap: +54 KB in `.rodata` (Bluetooth SIG company-name table), +41 KB in `.text` (bluetooth lookup functions), +11 KB in `.ARM.extab`. Post-fix, 16 other sections went byte-identical with upstream; SBFspot NEEDED dropped `libbluetooth.so.3` to match. Gap collapsed from 110,604 B (8.9%) to **16,376 B (1.3%) — 85% closure**. Residual 16 KB is dominated by `.ARM.extab` (+11.5 KB) and explained by a **real Phase 3.3 discovery**: upstream V3.9.12 is **cross-compiled from Windows**. The upstream binary embeds eight `__FILE__` strings like `d:\rpi\cross\bookworm\gcc12.2.0\arm-linux-gnueabihf\sysroot\usr\include/boost/format/alt_sstream_impl.hpp` — drive letter, mixed separators, pointing at a Windows-hosted cross-toolchain's private sysroot. The `.comment` reads `(Raspbian ... +rpi1)` because the cross-gcc was built from Raspbian's source, but its `libstdc++.a` and `libbluetooth.a` bytes aren't ours and can't be recovered from public data. Phase 3.2's "Raspbian build-farm state" attribution was wrong in mechanism; corrected in `docs/phase3.3-baseline.md`, which also supersedes the Phase 3.2 explanation. Bar 1a' (pipeline-internal reproducibility): met. Bar 1a'' (upstream byte-match): ~98.7% achieved, residual 1.3% documented as Windows-cross-compile irreducible with concrete evidence. Probe workflow `.github/workflows/toolchain-probe.yml` preserved as Phase 3.2 artefact; release.yml reflects the current Phase 3.3 state. **Next gate: Phase 4 — back-test V3.9.11 and V3.9.10 against Bar 1a' on the current pipeline.**
...
- `docs/phase3.3-baseline.md` — Phase 3.3 end state: libbluetooth static-link closes 85% of the 110 KB delta; per-section diff + embedded `d:\rpi\cross\...` strings identify upstream as a Windows cross-compile. Phase 3 close.
