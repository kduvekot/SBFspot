...
Phase 3.5 complete — cross-combo generalisation probe run. Five probe iterations in `.github/workflows/phase35-probe.yml` tested the Phase 3.4 technique across all three matrix axes (last run `24765262702`, 5 cells, frozen behind `.phase35-probe-trigger`). Results: **2 of 5 cells clean byte-match** (`nosql × arm × bullseye`, `sqlite × arm64 × bullseye`); **2 cells close-but-not-clean** with small residuals (`mariadb × arm × bullseye` has ~0.1% byte drift from `libmariadbclient.a` object-byte differences with upstream's private sysroot; `sqlite × arm × buster` has `--as-needed` subtleties that drop libdl/libgnutls from our NEEDED where upstream keeps them); **1 cell known irreducible** (`sqlite × arm64 × bookworm` carries the Phase-1-flagged dual-`.comment` where upstream links a pre-built Raspbian-tagged static lib into a Debian build — ~9.7 KB `.text` residual on SBFspot, daemon matches). **Revised Bar 1a'' status: achievable on 9 of 15 combos** (all non-bookworm) via per-combo LDFLAGS tweaks (static boost + explicit transitive-dep libs for mariadb, `-Wl,--no-as-needed -ldl -lgnutls` for buster, per-codename prefix-map); **6 of 15 combos** (all `*-bookworm`) carry an additional irreducible from upstream's private pre-built static libs we can't reproduce from public data. Phase 3.5 also uncovered real pipeline bugs the bullseye probe had masked: upstream statically links boost on ALL 15 tarballs (not just bookworm); upstream statically links libmariadbclient; buster needs `libcurl4-gnutls-dev` + legacy mirror; mariadb build needs `libmariadb-dev-compat`. Full per-cell data + the revised matrix table in `docs/phase3.5-baseline.md`. Windows-CI path reconfirmed out-of-scope: wouldn't help with the bookworm residuals (those are private sysroot artefacts, not host-OS artefacts). **Next gate: Phase 4 — back-test V3.9.11 / V3.9.10 against Bar 1a' on the current `arm × bookworm × sqlite` pipeline.**
